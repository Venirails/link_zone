...
class EventsController < ApplicationController
  ...
  def create
    ...
    @event.event_datetime = params[:event][:event_datetime].in_time_zone.to_date
    if @event.save
       @event.create_activity(:create, owner: current_user, recipient: @event.user, read_status: false)
  ...
  def calendar
    if request.xhr?
      friend_events = Event.select("events.*").joins("INNER JOIN friendships ON events.user_id = friendships.friendable_id").where("friendships.friend_id = #{current_user.id} AND friendships.friendable_type ='User'")
      ...
      @events = friend_events | current_user_events
    end
    respond_to do |format|
      format.html
